...
# model = TFBertModel.from_pretrained("bert-base-uncased")from indicnlp.tokenize import indic_tokenize

# Tokenize the text into words
words = indic_tokenize.trivial_tokenize(source[0])
...
def get_word_embeddings(words):
    ...
    # Only select the token embeddings (excluding [CLS] and [SEP])
    word_embeddings = embeddings[0][1:-1]

    # Average the embeddings in case the word is tokenized into subwords
    averaged_embedding = word_embeddings.mean(dim=0)
    return averaged_embedding

# Get and print the embedding for each word
for word in words:
    embedding = get_embedding(word)
    print(f'Token: {word}, Word Embedding: {embedding}')
    ...
    # word_embeddings # Specify the sequence length (e.g., the number of tokens in the input)
