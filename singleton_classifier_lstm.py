...
from tensor_builders.lstm_c import LSTMTensorBuilder
...
class SingletonClassifierModelBuilder:
    def __init__(self, use_words_feature: bool = True, use_context_feature: bool = True,
        ...
        if self.use_words_feature or self.use_context_feature:
            self.lstm_tensor_builder = LSTMTensorBuilder(vocab_size=len(embedding_matrix), vector_size=len(embedding_matrix[0]),
    ...
    def create_model(self, softmax: bool = True) -> Model:
        ...
        if self.use_context_feature:
            inp_prev, tensor_prev = self.lstm_tensor_builder.create_tensor()
            inp_next, tensor_next = self.lstm_tensor_builder.create_tensor()
        ...
        if self.use_syntactic_feature:
            ...
            # print(input_shape)
            # inp,tensor = self.lstm_tensor_builder.create_tensor(input_shape=(self.syntactic_features_num, ), 
            #                                                  output_size=16,dropout=0.2)
            # inputs.append(inp)
            # tensors.append(tensor)  
